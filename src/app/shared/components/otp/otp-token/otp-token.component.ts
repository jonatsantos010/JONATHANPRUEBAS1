...
export class OtpTokenComponent implements OnInit {
  ...
  get tokenTime(): string {
    ...
    return `${min.toString().padStart(2, '0')}:${this.secondstokenExpirationTime.toString().padStart(2, '0')}`;
  }

  get tokenTryAgainTime(): string {
    const min = Math.floor(this.tryAgainTime / 60);
    return `${min.toString().padStart(2, '0')}:${this.secondsTryAgainTime.toString().padStart(2, '0')}`;
  }
