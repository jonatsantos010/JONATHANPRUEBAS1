...
export class InputDateComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if ('minDate' in changes) {
      if (this.bsConfig) {
        this.bsConfig.minDate = this.minDate || new Date();
      }
    }
  }
  ...
  handleOutsideClick(e) {
    if (
      ...
      (e.target.offsetParent &&
        e.target.offsetParent.localName.includes('ngb-datepicker')) ||
      !(
        e.target.parentElement &&
        e.target.parentElement.parentElement &&
        !e.target.parentElement.parentElement.localName.includes(
          'ngb-datepicker'
        )
      )
    ) {
