...
export class KushkiFormComponent implements OnInit {
      ...
      next: (response: any): void => {
        ...
        if (!response.success) {
          this.result.emit({
            ...
            paymentInfo: {
              token: '',
              processId: this.paymentInfo.processId,
  ...
  saveInfo(): void {
    ...
    const payload = {
      ...
      email: this.paymentInfo.client.email,
      idPayment: this.paymentInfo.processId,
  ...
  private cardPaymentSubmit(): void {
    ...
    enum paymentType {
      subscription = 'requestSubscriptionToken',
      unique = 'requestToken'
    }

    this.kushki[paymentType[this.paymentInfo.payment.type]](
      {
  ...
  private processPayment(tokenInfo: ITokenResponse | ITokenOTPResponse | IToken3DSResponse | ITokenErrorResponse): void {
    const payload = {
      idProceso: this.paymentInfo.processId,
  ...
  // tslint:disable-next-line:max-line-length
  private validateCardPayment(tokenInfo: ITokenResponse | ITokenOTPResponse | IToken3DSResponse | ITokenErrorResponse): void {
    this.emitValues(tokenInfo, true);
    return;

    switch (tokenInfo['secureService']) {
      case 'KushkiOTP':
        this.validateOTP(tokenInfo as ITokenOTPResponse);
        break;
      case '3dsecure':
        this.validate3DS(tokenInfo as IToken3DSResponse);
        break;
    }
  }
  ...
  private emitValues(info: any, success: boolean): void {
    this.result.emit({
      success,
      message: '',
      paymentInfo: {
