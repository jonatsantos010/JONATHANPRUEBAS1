...
import {
  ...
  IDeleteRecipientRequest,
  IDeleteRecipientResponse,
  IGetRecipientsResponse,
  IRecipient,
  IUpdateRecipientResponse,
...
export class InternalRecipientsComponent implements OnInit {
  recipientForm: FormGroup = this.builder.group({
    ...
    email: [
      '',
      [Validators.pattern(RegularExpressions.email), Validators.required],
    ],
    cellPhone: [
      '',
      [
  ...
  private formValueChanges(): void {
    this.searchFilterControl.valueChanges.subscribe((value: string) => {
      this.currentPage = 1;
      this.recipientFiltering$ = this.recipients$.filter(
        (obj: IRecipient) =>
          obj.correo.toLowerCase().includes(value.toLowerCase()) ||
          obj.nombres.toLowerCase().includes(value.toLowerCase()) ||
          obj.telefono.toLowerCase().includes(value.toLowerCase())
      );
    });

    this.recipientFormControl['cellPhone'].valueChanges.subscribe(
      (value: string): void => {
        if (!value) {
          return;
        }
        ...
        if (
          this.recipientFormControl['cellPhone'].hasError('pattern') ||
          +value.slice(0, 1) != 9
        ) {
          this.recipientFormControl['cellPhone'].setValue(
    ...
    this.emailConfigurationService.getRecipients().subscribe({
      next: (response: IGetRecipientsResponse): void => {
        ...
        this.recipientFiltering$ = this.recipients$ =
          response.listaCorreos.filter(
            (x: IRecipient): boolean => +x.tipo == 2
          );
      },
  ...
  private createRecipient(): void {
    ...
    this.emailConfigurationService
      .createRecipient(this.createRecipientPayload)
      .subscribe({
        next: (response: ICreateRecipientResponse): void => {
          ...
          this.response$ = {
            ...
            message: this.validateResMessage(response.correoCreado),
            showImage: true,
  ...
  validateResMessage(data: string) {
    const messageCode = {
      '0': 'Se guardó correctamente la información',
      '1': 'El dominio del correo no es válido',
      '2': 'El tipo de correo no es válido',
      '3': 'El correo ya existe',
      '4': 'Error de base de datos',
    };

    return messageCode[data] ?? 'Ocurrió un problema al guardar la información';
  }
  ...
  private updateRecipient(): void {
    ...
    this.emailConfigurationService
      .updateRecipient(this.createRecipientPayload)
      .subscribe({
        next: (response: IUpdateRecipientResponse): void => {
          ...
          this.response$ = {
            ...
            message: response.success
              ...
              : 'Ocurrió un problema al actualizar la información',
            showImage: true,
            reloadList: response.success,
          };
          this.vcr.createEmbeddedView(this.modalResponse);
    ...
    this.emailConfigurationService.deleteRecipient(payload).subscribe({
      next: (response: IDeleteRecipientResponse): void => {
        ...
        this.response$ = {
          success: response.success,
          message: response.success
            ? 'El correo se eliminó correctamente'
            : 'Ocurrió un problema al eliminar el correo',
          showImage: true,
          reloadList: response.success,
        };
        this.vcr.createEmbeddedView(this.modalResponse);
