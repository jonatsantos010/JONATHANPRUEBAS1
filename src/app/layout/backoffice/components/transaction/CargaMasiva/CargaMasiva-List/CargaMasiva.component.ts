  ...
  */
  ...
  getDataOfDocument(): void {
    if (this.fc['documentNumber'].valid) {
      ...
      this.spinner.show();
      const data: DocumentRequest = new DocumentRequest({
        type: this.fc['documentType'].value,
        documentNumber: this.fc['documentNumber'].value,
      });
      this._documentService.dataOfDocument(data).subscribe(
        (res: DocumentResponse) => {
          this.fc['razonSocial'].setValue(res.p_SLEGALNAME);
          this.fc['apellidoPaterno'].setValue(res.p_SCLIENT_APPPAT);
          this.fc['apellidoMaterno'].setValue(res.p_SCLIENT_APPMAT);
          this.fc['nombres'].setValue(res.p_SCLIENT_NAME);
          this.fc['direccion'].setValue(res.p_SADDRESS);
          this.fc['email'].setValue(res.p_SMAIL);
          this.fc['celular'].setValue(res.p_SPHONE);
          this.fc['clientState'].setValue(Number(res.p_SISCLIENT_GBD) === 1);
          this.fc['sexo'].setValue(
            Number(res.p_SSEXCLIEN) !== 0 ? Number(res.p_SSEXCLIEN) : null
          );
          this.setUbigeo();
          if (
            ...
            this.fc['nacionalidad'].setValue(
              res.p_NNATIONALITY !== null ? res.p_NNATIONALITY : '1'
            );
            this.fc['departamento'].setValue(
              res.p_NPROVINCE !== null ? res.p_NPROVINCE : null
            );
            this.fc['provincia'].setValue(
              Number(res.p_NLOCAT) !== 0 ? Number(res.p_NLOCAT) : null
            );
            this.fc['distrito'].setValue(
              Number(res.p_NMUNICIPALITY) !== 0
                ? Number(res.p_NMUNICIPALITY)
                : null
            );
          }
          if (Number(this.fc['documentType'].value) === 4) {
            this.fc['nacionalidad'].setValue(
              res.p_NNATIONALITY !== null ? res.p_NNATIONALITY : null
            );
            this.fc['departamento'].setValue(
              res.p_NPROVINCE !== null ? res.p_NPROVINCE : '14'
            );
            this.fc['provincia'].setValue(
              Number(res.p_NLOCAT) !== 0 ? Number(res.p_NLOCAT) : 1401
            );
            this.fc['distrito'].setValue(
              Number(res.p_NMUNICIPALITY) !== 0
                ? Number(res.p_NMUNICIPALITY)
  ...
  exportarExcel(): void {
    this._CargaMasivaService.exportarExcel(
      this.DATA_CARGA_MASIVA.cargaMasivaDetail,
      'CargaMasiva'
    );
  }
