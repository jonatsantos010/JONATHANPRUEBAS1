...
export class PanelInfoContratanteComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.filtroEstados = [
      this.CONSTANTS.TIPO_DOCUMENTO.DNI
    ];
  ...
  onClear(tipo: any) {
    ...
    if (tipo === 1) {
      // this.contratante.tipoDocumento = {}; // AGF 12052023 Beneficiarios VCF
  ...
  async buscarContratante() {
    ...
    await this.clientInformationService.getCliente360(params).toPromise().then(
      async resp => {
