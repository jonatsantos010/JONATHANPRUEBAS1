  ...
  */
    ...
    if (detalle != '0') {
      ...
      this.clientInformationService.getCliente360(data).subscribe(
        async res => {
          ...
          const modalRef = this.modalService.open(EndorseeViewComponent, { size: 'lg', backdropClass: 'light-blue-backdrop', backdrop: 'static', keyboard: false });
  ...
  changeValidators() {
    ...
    if (this.mainFormGroup.controls.searchMode.value == "1") {
      if (this.mainFormGroup.controls.documentType.value == "2") { //modo: Por documento, tipodoc:dni
        ...
        this.mainFormGroup.controls.documentNumber.setValidators([Validators.required, Validators.maxLength(11), Validators.minLength(11), GlobalValidators.rucNumberValidator20]);
        this.mainFormGroup.controls.documentNumber.updateValueAndValidity();
  ...
  private createForm() {
    this.mainFormGroup = this.mainFormBuilder.group({
      documentNumber: ["", [Validators.required, Validators.maxLength(11), Validators.minLength(11), GlobalValidators.rucNumberValidator20]],
  ...
  firstSearch() {
    ...
    if (this.mainFormGroup.valid) {
      ...
      if(this.mainFormGroup.controls.documentNumber.value == "" && this.mainFormGroup.controls.legalName.value == ""){
        this.listar();
      }else{
      let errorList = [];
      if (this.mainFormGroup.controls.searchMode.value == "1") {
  ...
  pageChanged(page: number) {
    ...
    this.listToShow2 = this.foundResults.slice(((this.currentPage - 1) * this.itemsPerPage), (this.currentPage * this.itemsPerPage));
  }
  ...
  async processFirstSearch() {
    ...
    if (this.mainFormGroup.controls.searchMode.value == "1") {
      ...
      data.P_SIDDOC = this.mainFormGroup.controls.documentNumber.value.toString().trim();
    } else {
      if (this.mainFormGroup.controls.legalName.value.trim() != '') {
        data.P_NIDDOC_TYPE = "";
        data.P_SIDDOC = "";
        data.P_SLEGALNAME = this.mainFormGroup.controls.legalName.value.toString().toUpperCase().trim();
      } else {
        let msjError = ""
          ...
          if (res.EListClient != null && res.EListClient.length > 0) {
            if (res.EListClient[0].P_SCLIENT != null) {
              if (res.EListClient[0].P_SIDDOC != null) {
                      ...
                      if (res_2.GENERICLIST != null) {
                        ...
                        if (this.listToShow2.length > 0) {
                          ...
                          if (this.listToShowAnul.length ==  0){
                            ...
                            Swal.fire("Información", "El endosatario fue previamente anulado, por favor comunicarse con soporte.", "error");
                            this.isLoadingScreenNotVisible = true;
                          }
                ...
                Swal.fire("Información", "El endosatario no cuenta con el nro de documento.", "error");
                this.isLoadingScreenNotVisible = true;
              }
              ...
              if (this.mainFormGroup.controls.searchMode.value == "1") {
                Swal.fire({
                  ...
                  if (result.value) {
                    ...
                    this.router.navigate(['/extranet/add-contracting'], { queryParams: { typeDocument: this.mainFormGroup.controls.documentType.value, document: this.mainFormGroup.controls.documentNumber.value, receiver: "mantenimiento-endosatario", code: "3" } });
                  } else if (result.dismiss === Swal.DismissReason.cancel) {
                  }
                });
              } else {
            ...
            if (this.mainFormGroup.controls.documentType.value.toString() == "1") {//&& this.mainFormGroup.controls.documentType.value.toString() != "2") {
              Swal.fire({
                ...
                if (result.value) {
                  ...
                  this.router.navigate(['/extranet/add-contracting'], { queryParams: { typeDocument: this.mainFormGroup.controls.documentType.value, document: this.mainFormGroup.controls.documentNumber.value, receiver: "mantenimiento-endosatario", valorEndorsee: 1, code: "3" } });
                } else if (result.dismiss === Swal.DismissReason.cancel) {
                }
              });
            } else {
  ...
  async listarTodo() {
    ...
    await this.clientInformationService.getProviderList(data2).subscribe(rest => {
  ...
  async listarAnulado() {
    ...
    await this.clientInformationService.getProviderList(data3).subscribe(rest => {
      this.foundResults = rest.providerList;
      if (this.foundResults != null && this.foundResults.length > 0) {
        this.totalItems = this.foundResults.length;
        this.listToShowAnul = this.foundResults.slice(((this.currentPage - 1) * this.itemsPerPage), (this.currentPage * this.itemsPerPage));
      }
      else {
        this.totalItems = 0;
      }
  ...
  async EliminarEndorsee(item) {
    ...
    Swal.fire({
      ...
      if (!result.dismiss) {
        ...
        await this.clientInformationService.deltProvider(data).subscribe(rest => {
