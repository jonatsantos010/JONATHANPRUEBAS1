...
export class RequestQuoteComponent {
    ...
    clickSearch() {
        ...
        this.cotizacionesParams = {
            ...
            NumCredito: this.filters.numCredito || '',
    ...
    typeSearch(type) {
        let filtroContratante = (this.filters.contratante.tipoBusqueda || {})
            .codigo;
            ...
            if (filtroContratante !== '') {
                if (
                    ...
                    if (
                        ...
                        (filtroContratante == 'POR_NOM' &&
                            !!this.filters.contratante.nombres)
                    ) {
                        return '2';
                    } else {
                        return '3';
        ...
        if (type == 'BrokerSearchMode') {
            if (filtroBK !== '') {
                if (filtroBK == 'POR_DOC' && !!this.filters.broker.numDocumento) {
                    return '1';
                } else {
                    if (
                        (filtroBK == 'POR_NOM' && !!this.filters.broker.razonSocial) ||
    ...
    getValidations(): any {
        ...
        if (
            !!this.filters.broker.tipoDocumento &&
            !!this.filters.broker.numDocumento &&
            this.filters.broker.numDocumento.trim().length > 0
        ) {
            if (
                this.filters.broker.tipoDocumento.Id === 1 &&
                CommonMethods.validateRucAP(this.filters.broker.numDocumento)
            ) {
                msg += 'El número de RUC del broker no es válido <br>';
            }
            if (
                this.filters.broker.tipoDocumento.Id === 2 &&
                CommonMethods.validateDNI(this.filters.broker.numDocumento)
            ) {
                msg += 'El número de DNI del broker no es válido <br>';
            }
            if (
                this.filters.broker.tipoDocumento.Id === 4 &&
                CommonMethods.validateCE(this.filters.broker.numDocumento)
            ) {
                msg += 'El número de CE del broker no es válido <br>';
            }
            if (
                this.filters.broker.tipoDocumento.Id === 6 &&
                CommonMethods.validatePass(this.filters.broker.numDocumento)
            ) {
