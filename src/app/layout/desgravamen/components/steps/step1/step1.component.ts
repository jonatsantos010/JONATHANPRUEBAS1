...
import { DocumentService } from '@root/shared/services/document/document.service';
import { DocumentFormatResponse, DocumentRequest } from '../../../../../shared/models/document/document.models';
  ...
  getDataOfDocument(): void {
    if (this.f['documentNumber'].valid) {
      const data: DocumentRequest = new DocumentRequest({
        type: this.f['documentType'].value,
        documentNumber: this.f['documentNumber'].value
      });
      this._spinner.show();
      this.subscription = this._documentService.dataOfDocument(data).subscribe(
        (res: any) => {
          this.dataOfDocument = new DocumentFormatResponse({
            ...res,
            success: true
