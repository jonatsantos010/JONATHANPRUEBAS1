...
import { VcfReportService } from '../../../shared/services/vcf-report/vcf-report.service';
...
export class VcfReserveReportComponent implements OnInit {
    ...
    //Formato de la fecha
    constructor(
        private VcfReportService: VcfReportService,
    ...
    exportToExcel() {
        if (this.foundResults != null && this.foundResults.length > 0)
            this.excelService.generateReportTecExcel(this.foundResults, 'ReporteTecnicoVDP');
        else
                ...
                if (this.isError == true) {
                    ...
                    swal.fire({
                        ...
                        if (result.value) {
                            ...
                            this.VcfReportService.ProcessReserve(data).subscribe(
                                res => {
                                    this.foundResults = res.genericResponse;

                                    if (this.foundResults != null && this.foundResults.length > 0) {
                                        this.totalItems = res.totalRowNumber;
                                        this.excelService.generateVcfReserveRegistryReport(this.foundResults, 'Reporte Reservas Reaseguro VCF -');
                                    }
                                    else {
                                        this.totalItems = 0;
