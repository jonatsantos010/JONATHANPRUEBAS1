  ...
  implements OnInit, OnDestroy, AfterViewChecked
{
  ...
  get validarStep(): boolean {
    ...
    this.dataAsegurados.forEach((e) => {
      sumPercentageValid = e.beneficiarios?.map((x) => x.porcentaje)?.reduce((a, b) => +a + +b, 0);
    });
  ...
  getParentesco(id: any): string {
    ...
    return this.parametros$.parentescos.find((x) => Number(x.id) === Number(id))
      ?.descripcion;
  }
  ...
  private infoDocumento(token: string): void {
    ...
    this.utilsService.documentInfoResponse(data).subscribe({
      next: (res: DocumentInfoResponseModel) => {
        ...
        if (
          ...
          this.fb['sexo'].setValue(infoClient?.sex);
        ...
        if (
          ...
          this.fb['fechaNac'].setValue(
            this.insurance.fechaNac || this.insurance.birthDate
          );
        } else {
    ...
    this.clientInfoService.getPlans(payload).subscribe({
      next: (response: any) => {
        ...
        if (response.errores?.length) {
          this.listErrorEstructura$ = response.errores.map((obj: any) => ({
            descripcion: obj.mensaje,
            numeroFila: obj.registro,
          }));
  ...
  formatoTrama(): void {
    ...
    this.utilsService.fetchResource(
