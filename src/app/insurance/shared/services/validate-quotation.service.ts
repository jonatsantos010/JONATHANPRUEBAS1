...
export class ValidateQuotationService {
  ...
  getDataOfDocument(data): Observable<Array<DocumentResponse>> {
    const url = `${this.API_URI}/Emission/cliente/${data.tipoDocumento}/${data.numeroDocumento}`;
    const call = this._http.get(url);
    const data$: Observable<Array<DocumentResponse>> = new Observable((obs) => {
      call.subscribe(
        (res: Array<DocumentResponse>) => {
          obs.next(res);
          obs.complete();
        },
        (err: any) => {
