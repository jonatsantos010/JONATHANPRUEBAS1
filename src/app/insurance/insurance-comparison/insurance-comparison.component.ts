...
export class InsuranceComparisonComponent implements OnInit, AfterViewInit {
  ...
  productSelected = JSON.parse(sessionStorage.getItem('_producto_selecionado')) ?? {};
  ...
  nextStep(): void {
    ...
    this.sessionPlanSelected?.plan.coberturas.filter((x) => x?.selected || x?.obligatoria === '1').forEach((e) => {
      const cobs: any = {
        id: e.id?.toString(),
    ...
    this.sessionPlanSelected?.plan.beneficios.filter((x) => x?.selected).forEach((e) => {
      const benf: any = {
        id: e.id?.toString(),
        descripcion: e.descripcion,
    ...
    this.sessionPlanSelected?.plan.asistencias.filter((x) => x?.selected || productSelected.key == 'individual_hits').forEach((e) => {
      const asist: any = {
        id: e.id?.toString(),
        descripcion: e.descripcion,
        seleccionado: Number(e.selected) === 1,
